...
export default async function Unsubscribe({ searchParams }: { searchParams: Promise<{ w?: string; s?: string; t?: string; u?: string; done?: string }> }) {
  ...
  const what = t === "digest" ? "bản tin deal qua email" : t === "sale" ? "email nhắc ngày sale" : "theo dõi giá sản phẩm này";
  return (
    <div className="auth-card">
      ...
      {done ? (
        <>
          <h1>Đã huỷ</h1>
          <p className="muted">Bạn sẽ không nhận {done === "digest" ? "bản tin deal" : done === "sale" ? "email nhắc sale" : "email về sản phẩm này"} nữa. Có thể bật lại trong trang Sở thích.</p>
